...
\section{Rules for java}

\section{General overview}

\begin{chunk}{java/Tangle.java}
...
\getchunk{java/pass through codec}
\getchunk{java/mapped file input}
\getchunk{java/chunk reader}
\getchunk{java/latex extractor}
\getchunk{java/output}
\getchunk{java/exceptions}
...
\begin{chunk}{java/latex extractor}
...
\getchunk{java/latex parser}
\getchunk{java/latex reader states}
}
\end{chunk}

\subsection{The latex parser}
\begin{chunk}{java/latex parser}
private class Parser {
    ...
    private void parse() {
        ...
        while (beginChunkCommandMatcher.find(start)) {
            ...
            while(inChunkCommandMatcher.find(start)) {
...
\begin{chunk}{python/tangle.py}
def get_chunk(pamphlet, chunk_name):
    raw_chunk = extract(pamphlet, chunk_name)
    return expand_getchunk(pamphlet, raw_chunk)
\end{chunk}

\begin{chunk}{python/tangle.py}
def extract(pamphlet, chunk_name):
    with pamphlet() as lines:
        try:
            while True:
                skip_to_begin_chunk(lines, chunk_name)
                yield from get_to_end_chunk(lines)
        except EndOfInput:
            pass
\end{chunk}

\begin{chunk}{python/tangle.py}
def skip_to_begin_chunk(lines, chunk_name):
    pattern = r"^\\begin{chunk}\{%s}$" % re.escape(chunk_name)
    for line in lines:
...
\begin{chunk}{python/tangle.py}
def expand_getchunk(pamphlet, lines):
    for line in lines:
        getchunk = re.match(r"\\getchunk\{([^}]*)}$", line)
        if getchunk:
            yield from get_chunk(pamphlet, getchunk.group(1))
        else:
            yield line
\end{chunk}
